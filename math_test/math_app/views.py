...
from .models import Problem, Hint, Tag, Grade, UserProfile, Subject
...
class ProblemCreateView(LoginRequiredMixin, CreateView):
    """新しい数学問題を作成"""
    model = Problem
    fields = ['grade', 'title', 'image', 'hint_approach', 'hint_formula', 'hint_technique', 'tags']
...
class ProblemUpdateView(LoginRequiredMixin, UpdateView):
    model = Problem
    fields = ['grade', 'title', 'hint_approach', 'hint_formula', 'hint_technique', 'tags']
    template_name = 'math_app/problem_form.html'
    login_url = 'login'
